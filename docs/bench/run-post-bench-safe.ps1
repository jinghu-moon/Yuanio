...
for ($i = 1; $i -le 10; $i++) {
  ...
  $relayOut = "docs/bench/protocol-e2e-post-run$($i)-relay.out.log"
  $relayErr = "docs/bench/protocol-e2e-post-run$($i)-relay.err.log"
  ...
  try {
    if (-not (Wait-RelayHealthy -serverUrl $server -timeoutSec 20)) {
      throw "run $i relay health timeout"
    }

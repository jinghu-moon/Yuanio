...
function loadWorkflowState(cwd = process.cwd(), policyObj = DEFAULT_POLICY) {
  ...
  if (!fs.existsSync(sessionPath)) {
    return { exists: false, sessionPath, sessionRelPath, sessionRelPaths, phase: "", nextAction: "",
             updatedAt: "", isDone: false, isStale: false, hasPending: false };
  }

  let raw = "";
  try { raw = fs.readFileSync(sessionPath, "utf8"); }
  catch {
    return { exists: true, sessionPath, sessionRelPath, sessionRelPaths, phase: "", nextAction: "",
             updatedAt: "", isDone: false, isStale: false, hasPending: true, parseError: true };
  }

  const fields     = parseSessionFields(raw);
  const phase      = normalizeToken(fields.current_phase ?? fields.phase ?? "");
  const nextAction = normalizeToken(fields.next_action   ?? fields.next  ?? "");
  const updatedAt  = String(fields.updated_at ?? fields.updated ?? "").trim();
  const updatedMs  = Date.parse(updatedAt);

  const donePhases  = (Array.isArray(wf.donePhases)      ? wf.donePhases      : ["done"]).map(normalizeToken);
  const doneActions = (Array.isArray(wf.doneNextActions)  ? wf.doneNextActions : ["done"]).map(normalizeToken);
  const maxMs       = (Number(wf.maxActiveHours) || 72) * 3_600_000;
  ...
  return { exists: true, sessionPath, sessionRelPath, sessionRelPaths, phase, nextAction, updatedAt,
           isDone, isStale, hasPending, fields, parseError: false };
}
...
function countLedgerNodes(cwd) {
  ...
  try {
    ...
    for (const line of lines) {
      // Match:  ### N3 ✅ …  or  ### N3 ✓ …
      const m = line.match(/^###\s+(N\d+|S\d+)\s+[✅✓]/u);

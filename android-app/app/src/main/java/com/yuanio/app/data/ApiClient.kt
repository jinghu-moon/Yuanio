...
class ApiClient(private val serverUrl: String) {
    ...
    /** 注册 FCM token（HTTP 入口，作为 Socket 注册兜底） */
    suspend fun registerPushToken(currentToken: String, fcmToken: String): Boolean = withContext(Dispatchers.IO) {
        if (currentToken.isBlank() || fcmToken.isBlank()) return@withContext false
        val body = JSONObject()
            .put("token", fcmToken)
            .toString()
            .toRequestBody(json)
        ...
        val req = Request.Builder()
            .url("$serverUrl/api/v1/push/register")
            .header("Authorization", "Bearer $currentToken")
            .post(body)
            .build()
        ...
        runCatching { JSONObject(text).optBoolean("registered", true) }.getOrDefault(true)
    }
